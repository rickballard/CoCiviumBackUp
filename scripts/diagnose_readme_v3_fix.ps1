...
cmd /c "git show --no-textconv HEAD:README.md > __head.tmp 2>nul"
cmd /c "git show --no-textconv origin/main:README.md > __orig.tmp 2>nul"

if(-not (Test-Path '__head.tmp') -or -not (Test-Path '__orig.tmp')){
  throw "Failed to capture one or both files from git."
}
