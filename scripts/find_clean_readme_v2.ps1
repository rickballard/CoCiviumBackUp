...
function S([int[]]$codes){ ($codes | % { [string][char]$_ }) -join '' }

# Codepoint-built tokens (ASCII-safe to embed in this script)
$C3    = S 0x00C3                 # '?'
$C2    = S 0x00C2                 # '?'
...
function Get-ReadmeUTF8([string]$spec){
  ...
  try { [Text.UTF8Encoding]::new($false,$true).GetString($bytes) } catch { $null }
}

function Measure([string]$txt){
...
$rows = foreach($sha in $commits){
  ...
  $m = Measure $txt
  if($m){ [pscustomobject]@{ Sha=$sha; Meta=(git show -s --format="%h %ci  %an  %s" $sha); A=$m.A; B=$m.B; Combo=$m.Combo; Preview=$m.Preview } }
}

Write-Host "`n=== Tail (last 6 commits) ==="
$rows | Select-Object -Last 6 | % { "{0}  A={1} B={2} Combo={3}" -f $_.Meta,$_.A,$_.B,$_.Combo } | Write-Output

# "Clean" = no explicit U+FFFD-doubling token and very low noise for ?/?
